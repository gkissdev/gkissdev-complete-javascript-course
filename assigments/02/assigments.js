...
const myCountry = {
    ...
    language: 'english',
    population: 60,
    neighbours: ['GERMANY', 'ITALY', 'HUNGARY'],
    checkIsland: function () {
        this.isIsland = this.neighbours === 0 ? true : false
        return this.isIsland
    },
    describe: function () {
        this.description = `${this.country} has ${this.population} million 
...
console.log(myCountry.describe() + `${myCountry.checkIsland() ? ` and is an island` : `and is not an island`}`);
...
for (let i = 0; i < percentages2.length; i++) {
    if (percentages[i] !== percentages2[i]) {
        console.log(`Error at ${percentages[i]} index percentages: ${i} percentages2: ${percentages2[i]}`);
        isValid = false;
        break;
    }
}
    ...
    if (listOfNeighbours[i].length > 1) {
        for (let j = 0; j < listOfNeighbours[i].length; j++) {
            for (let k = 0; k < listOfNeighbours[i].length; k++) {
                if (j !== k)
                    console.log(`Neighbour country for ${listOfNeighbours[i][j]}: ${listOfNeighbours[i][k]}`);
            }
        }
    } else {
        console.log(`Country ${listOfNeighbours[i]} have no neighbour`);
    }
}
...
for (let i = 0; i < percentages3.length; i++) {
    if (percentages[i] !== percentages3[i]) {
        console.log(`Error at ${percentages[i]} index percentages: ${i} percentages2: ${percentages3[i]}`);
        isValid = false;
        break;
    }
}
